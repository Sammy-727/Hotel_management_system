...
def protect():
    ...
    # request.endpoint can be None for some requests; ensure safe check
...
def add_booking():
    cid, rid = request.form["customer_id"], request.form["room_id"]
    checkin, checkout = request.form["checkin"], request.form["checkout"]
...
def service():
    if request.method == "POST":
        bid, iid, qty = int(request.form["booking_id"]), int(request.form["item_id"]), int(request.form["quantity"])
